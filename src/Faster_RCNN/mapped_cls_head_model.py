...
class ClassificationModule(nn.Module):
    def __init__(self, out_channels, n_classes, roi_size, hidden_dim=512, p_dropout=0.3):
        ...
        self.counts = {n: 0 for n in range(n_classes)}
        ...
        # self.activ2param = nn.Linear(hidden_dim, hidden_dim)
        self.activ2param = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim))

        self.protos = nn.Linear(hidden_dim, n_classes).weight.detach()
    ...
    def forward(self, feature_map, proposals_list, gt_classes=None):
        ...
        if mode == 'train':
            with torch.no_grad():
                for activ, gt_class in zip(out, gt_classes):
                    gt_class = int(gt_class)
                    if gt_class == -1:
                        continue
                    self.counts[gt_class] += 1
                    self.protos[gt_class] -= (activ - self.protos[gt_class]) / self.counts[gt_class]
            
        
        # # get the classification scores
        # cls_scores = self.cls_head(out)

        cls_scores = self.activ2param(out) @ self.protos.T
        
        if mode == 'eval':
            return cls_scores
        
        # compute cross entropy loss
        cls_loss = F.cross_entropy(cls_scores[gt_classes != -1], gt_classes[gt_classes != -1].long())
